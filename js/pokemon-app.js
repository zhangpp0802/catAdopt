...
let catKinds = ["Abyssinian Cat","American Bobtail Cat Breed","American Curl Cat Breed","American Shorthair Cat","American Wirehair Cat Breed","Balinese-Javanese Cat Breed","Bengal Cat","Birman Cat Breed"]
let gender = ["She", "He"]
let gameState = {
	points: 0,
	captured: [],
	...
	onEnterRange: (landmark, newLevel, oldLevel, dist) => {
		...
		if (newLevel == 2) {
			...
			if (!gameState.captured.includes(landmark.name)) {
				...
				// Add a message
				const randomcat = Math.floor(Math.random() * catKinds.length);
				const randomgender = gender[Math.floor(Math.random() * gender.length)];
				gameState.messages.push(`You adopted poor little ${landmark.name}! ${randomgender} is a ${catKinds[randomcat]}!`)
				display_image(randomcat.toString()+'.png', 
                 200, 
                 200, 
                 'JavaScriptImage');
